...
- **二上國語 L1～L6 平時卷生成 (115學年度)**:
  - 成功設計二上國語一～六課生字融入的平時卷，總分 100 分。
  - **傳統考卷版面重現**：首創 A4 橫向、左右雙欄、垂直雙列（共 2x3 包含右側標題欄）的直書（tbRl）結構。右側以垂直合併儲存格容納學校抬頭與班級/姓名欄位，並加上中央虛線摺線與 solid 實線框線，完美重現傳統印製考卷的視覺排版。
...
- **平時卷其餘課別擴展**:
  - 規劃並生成二上國語其餘課別（如七至十二課）的平時卷，維持相同的排版風格。
- **考試題型豐富度最佳化**:
  - 根據教學反饋，調整造句或選擇題內容，以符合班級學生的學習程度。
